...
| 2026-05-08 | (parent of this row's commit) | openai-codex / gpt-5.3-codex | PASS | PASS | PASS | PASS | PASS | PASS | PASS | Re-run after the auto-discovery fix: bonsai is now picked up by pi via the `.pi/extensions/context-bonsai` symlink at the pi-mono root, so users running `./pi-test.sh` (or any pi invocation in this workspace) get the bonsai tools registered automatically — no more manual `-e packages/context-bonsai` flag required. The e2e harness's redundant `-e packages/context-bonsai` was removed because it caused "tool conflicts" double-load errors against the auto-discovered extension. This run validates that the harness-without-explicit-flag path produces the same green result as the prior explicit-load path. Full run wall-time ≈ 3 min. |
